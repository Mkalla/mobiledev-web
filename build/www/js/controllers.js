app.controller("mainCtrl", function($state, $rootScope, $ionicPlatform, $ionicHistory){
  $rootScope.user = JSON.parse(window.localStorage.getItem("emergency"));
...
app.controller('CameraCtrl', function($ionicPlatform, $scope, $cordovaCamera, $ionicHistory) {
  $scope.vehicleImages = [];
  $scope.roadImages = [];
  $scope.witnessImages = [];
  ...
  $ionicPlatform.ready(function() {
    $scope.takePicture = function(sender){
      ...
      $cordovaCamera.getPicture(options).then(function(imgURI) {
        ...
        if(sender == "vehicle"){
          $scope.vehicleImages.push(imgURI);
        } else if(sender == "road") {
          $scope.roadImages.push(imgURI);
        } else if(sender == "witness") {
          $scope.witnessImages.push(imgURI);
        }
      }, function(err) {
        // error
        alert(err);
...
app.controller('registerCtrl', function($scope, $rootScope, $ionicSideMenuDelegate, $state){
  ...
  $scope.submit = function(){
    ...
    //Save data
    window.localStorage.setItem("emergency", [JSON.stringify($scope.user)]); //androids chrome is very picky. Needs to be an array
